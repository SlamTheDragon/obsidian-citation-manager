...
import { ReferenceMetadata, ProjectRecord, ProjectHealthStats, CitationManagerSettings, CitationStyle, InBodyFormat, ALL_PROJECTS_ID, CitationCollection, DEFAULT_COLLECTION, DEFAULT_COLLECTION_ID } from '../../backend/types';
...
export class CitationManagerView extends ItemView {
  ...
  private openDiagnosticId: string | null = null;
  ...
  private async openAttachedPDF(ref: ReferenceMetadata) {
    await CitationCardRenderer.openAttachedPDF(this.app, ref, this.settings.referencesFolder);
  }
    ...
    if (this.stats) {
      ...
      if (this.statsActiveTab === 'docs') {
        ...
        if (warningCount > 0) {
          ...
          // Simplified Diagnostic Accordion List in Side Panel (single open accordion)
          ...
          for (const w of this.stats.lintWarnings) {
            const isOpen = this.openDiagnosticId === w.id;
            ...
            header.addEventListener("click", () => {
              this.openDiagnosticId = (this.openDiagnosticId === w.id) ? null : w.id;
              this.renderUI();
            });
