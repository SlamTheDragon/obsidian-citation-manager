...
Obsidian Citation Manager provides strict, compliant implementations of 5 major academic style manuals.
...
## 2. In-Text Author Disambiguation Rules
...
## 3. Compound Multi-Citation Merging

When multiple citations occur together, they are compounded according to each manual's rules:

* **Author-Date Standards (APA 7, Harvard, Chicago)**:
  - Sorted alphabetically by primary author surname, then chronologically.
  - APA 7: `(Carter et al., 2026; Li, 2024; Norman, 2013)`
  - Harvard: `(Carter et al. 2026; Li 2024; Norman 2013)`

* **Numeric Standards (IEEE, Vancouver)**:
  - Sorted numerically by assigned citation index.
  - IEEE: `[1, 3, 5]`
  - Vancouver: `(1, 3, 5)`
...
For numeric standards (IEEE and Vancouver), sequential numbers are computed across all documents linked to a Citation Bucket:
1. All linked documents are scanned in registered order.
2. The first occurrence of each unique citekey is assigned sequential index `N`.
3. In-text citations and the compiled master bibliography strictly reflect this unified numbering.
